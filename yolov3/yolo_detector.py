...
def non_max_suppression(prediction, conf_thres=0.5, nms_thres=0.4):
    ...
    class_conf, class_pred = prediction[:, 5:].max(1, keepdim=True)
    ...
    while detections.size(0):
        ...
        _, max_idx = detections[:, 4].max(0)
        max_detection = detections[max_idx].unsqueeze(0)
        keep_boxes.append(max_detection)
        
        # Remove this detection
        detections = torch.cat([detections[:max_idx], detections[max_idx+1:]])
        
        if detections.size(0) == 0:
            break
            
        # Calculate IoU of this box with remaining boxes
        ious = bbox_iou(max_detection[:, :4], detections[:, :4])
        
        # Remove boxes with IoU > threshold
        detections = detections[ious.squeeze() < nms_thres]
        
    if keep_boxes:
        return torch.cat(keep_boxes)
    return torch.FloatTensor()
...
if __name__ == "__main__":
    ...
    # Print detections
    if detections.size(0) > 0:
        print(f"\nDetected {detections.size(0)} objects:")
        for det in detections:
            cls = int(det[6])
            conf = det[4]
